...
def extract_inline_audio(parsed: dict) -> Optional[bytes]:
    if not isinstance(parsed.get("serverContent"), dict): return None
    if not isinstance(parsed["serverContent"].get("modelTurn"), dict): return None
    ...
    for part in parts:
        ...
        if not isinstance(part.get("inlineData"), dict): continue
        b64_audio = part["inlineData"].get("data")
        if not isinstance(b64_audio, str) or not b64_audio: continue
        
        normalized = b64_audio.replace("-", "+").replace("_", "/")
        while len(normalized) % 4 != 0: normalized += "="
        try:
...
async def start_agent_loop():
    ...
    async def enqueue_audio(pcm: bytes):
        ...
        if not pcm or len(pcm) % 2 != 0 and not (pcm := pcm[:-1]): return
        try:
            await asyncio.wait_for(output_queue.put(pcm), timeout=0.5)
            bot_speaking_until = max(bot_speaking_until, asyncio.get_running_loop().time() + BOT_SPEAKING_HOLD_SEC)
    ...
    async def playback_worker():
        ...
        while not stop_event.is_set():
            try:
                ...
                pass
            if not started:
                if len(pending) < prebuffer_target: continue
                started = True
    ...
    try:
        async with websockets.connect(URL, max_size=20*1024*1024, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
            ...
            if not setup_ok: return
            agent_status = "live"

            async def sender_audio():
                while not stop_event.is_set():
                    try:
                        if not ENABLE_BARGE_IN and asyncio.get_running_loop().time() < bot_speaking_until:
                            await asyncio.sleep(0.02)
                            continue
                        data = await asyncio.to_thread(stream_in.read, INPUT_CHUNK, exception_on_overflow=False)
                ...
                try:
                    while not stop_event.is_set():
                        ...
                        if not ok:
                            ...
                            continue
                        ok_jpg, encoded = await asyncio.to_thread(cv2.imencode, ".jpg", frame, [int(cv2.IMWRITE_JPEG_QUALITY), VIDEO_JPEG_QUALITY])
                        if not ok_jpg:
                            await asyncio.sleep(VIDEO_INTERVAL_SEC)
                            continue
